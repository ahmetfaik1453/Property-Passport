      ...
      <div className="max-w-xl mx-auto w-full space-y-6">
        ...
        <div className="flex items-center justify-between gap-2">
          <Link href="/" className="flex items-center py-1 shrink-0">
            <Logo className="h-8 sm:h-12 w-auto" height={44} width={180} priority />
          </Link>
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            <Link href="/dashboard">
              <Button variant="ghost" size="sm" className="h-7 sm:h-8 px-2 sm:px-3 text-[11px] sm:text-xs font-semibold text-[#222222]">
                Pano
              </Button>
            </Link>
            <Badge variant="outline" className="gap-1 text-[10px] sm:text-[11px] font-mono border-[#dddddd] text-[#222222] px-2 py-0.5 whitespace-nowrap">
              <Lock className="w-3 h-3 text-emerald-600 shrink-0" /> <span className="hidden xs:inline">Korumalı </span>Doğrulama
            </Badge>
          </div>
        </div>
        ...
        <Card className="border border-[#dddddd] shadow-none bg-white rounded-[16px] overflow-hidden">
          ...
          <CardContent className="p-6 space-y-6 text-xs">
            ...
            {/* Immutability & Tamper-Proof Badge */}
            <div className="p-3.5 rounded-xl bg-emerald-50/70 border border-emerald-200/80 text-[11px] text-emerald-950 leading-relaxed">
              <p className="font-bold text-emerald-900 mb-1 flex items-center gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-700 shrink-0" />
                Değiştirilemez Dijital Mühür (Tamper-Proof)
              </p>
              Bu tutanak tamamlandıktan sonra sisteme kayıtlı <strong>{record.evidenceCount}</strong> üzerinde hiçbir tarafça sonradan silme, değiştirme veya yeni görsel ekleme işlemi yapılamaz. Tutanak oluşturulduğu andaki orijinal haliyle dondurulmuştur.
            </div>
          </CardContent>
        </Card>
